...
export const LockerRoomModal: React.FC<LockerRoomModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-xs animate-in fade-in duration-150">
      <div className="relative w-full max-w-xl bg-[#fae5b8] border-4 border-[#1a2238] shadow-[0_10px_0_0_#0a0f1d] p-6 rounded-xs text-[#5c3509]">
        ...
        <div className="flex items-center justify-between border-b-3 border-[#e2ba7d] pb-3 mb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-pixel text-lg sm:text-xl text-[#5c3509] tracking-wide">
              LOCKER ROOM
            </h2>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1.5 px-3 py-1 bg-[#fae9c8] border-2 border-[#d4a86a] font-pixel text-xs text-[#b45309]">
              <PixelCoin size={18} />
              <span>{coins.toLocaleString()}</span>
            </div>
            <button
              onClick={onClose}
              className="w-8 h-8 bg-[#b91c1c] text-[#fae5b8] border-2 border-[#1a2238] flex items-center justify-center cursor-pointer shadow-[0_2px_0_0_#450a0a]"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div className="bg-[#ebd2a4] border-3 border-[#c99a57] rounded-xs flex flex-col items-center justify-center p-5">
              ...
              <div className="grid grid-cols-3 gap-2">
                {HELMET_COLORS.map(item => (
                  <button
                    ...
                    className={`p-2 border-2 text-left cursor-pointer transition-all flex flex-col items-center justify-center relative ${
                  ...
                  >
                    <div
                      className="w-6 h-6 border border-black mb-1"
                      style={{ backgroundColor: item.color }}
                    />
                    <span className="font-retro text-[9px] text-[#5c3509] text-center leading-tight">
                      {item.name}
                    </span>
                    {item.cost > 0 && (
              ...
              <div className="flex flex-wrap gap-1.5">
                {JERSEY_NUMBERS.map(num => (
                  <button
                    ...
                    className={`w-9 h-9 font-pixel text-xs border-2 cursor-pointer flex items-center justify-center transition-all ${
                      avatar.number === num
        ...
        {/* Footer actions */}
        <div className="mt-5 pt-3 border-t-2 border-[#e2ba7d] flex justify-end gap-3">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-[#ebd2a4] border-2 border-[#c99a57] text-[#5c3509] font-pixel text-xs cursor-pointer hover:bg-[#fae9c8]"
          >
            CANCEL
          </button>
          <button
            onClick={handleSave}
            className="px-6 py-2.5 bg-[#12579b] border-2 border-[#0a2d52] text-[#fae5b8] font-pixel text-xs cursor-pointer shadow-[0_4px_0_0_#051a30] hover:bg-[#186abb] active:translate-y-1 active:shadow-none"
          >
            EQUIP & SAVE
          </button>
        </div>
