...
export const StatsMatchesModal: React.FC<StatsMatchesModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-xs animate-in fade-in duration-150">
      <div className="relative w-full max-w-2xl bg-[#fae5b8] border-4 border-[#1a2238] shadow-[0_10px_0_0_#0a0f1d] p-6 rounded-xs text-[#5c3509] max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between border-b-3 border-[#e2ba7d] pb-3 mb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-pixel text-base sm:text-lg text-[#5c3509] tracking-wider">
              LIVE STATS & MATCHES
            </h2>
          </div>
          <button
        ...
        <div className="mb-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {LIVE_MATCHES.map((match) => (
              ...
              >
                <div className="flex items-center justify-between font-retro text-[10px] text-[#784610] mb-1">
                  <span className="flex items-center gap-1 font-pixel text-[9px] text-[#b91c1c] animate-pulse">
                    ● {match.status.toUpperCase()}
                  </span>
                  <span>{match.periodLabel}</span>
                </div>
                <div className="flex items-center justify-between font-pixel text-xs text-[#5c3509] my-2">
                  <span>{match.homeTeam}</span>
                  <span className="text-base text-[#12579b]">{match.homeScore} - {match.awayScore}</span>
                  <span>{match.awayTeam}</span>
                </div>
                {match.recentEvent && (
                  <div className="text-[10px] font-retro text-[#b45309] bg-[#fae9c8] p-1.5 border border-[#d4a86a] mt-1">
                    ⚡ {match.recentEvent}
                  </div>
                )}
              </div>
        ...
        <div className="mb-5 p-4 bg-[#fae9c8] border-2 border-[#c99a57] rounded-xs">
          ...
          <div className="flex flex-wrap gap-2">
            {roster.slice(0, 4).map((player) => (
              <button
                ...
                className="px-3 py-1.5 bg-[#12579b] hover:bg-[#186abb] text-[#fae5b8] font-pixel text-[10px] border-2 border-[#0a2d52] cursor-pointer shadow-[0_2px_0_0_#051a30] active:translate-y-1 active:shadow-none"
              >
        ...
        <div>
          ...
          <div className="border-2 border-[#c99a57] overflow-hidden">
            <table className="w-full text-left font-retro text-xs">
